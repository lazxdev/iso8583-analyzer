import { FieldSpec, ISO_8583_1987_SPEC } from '../fields';

export interface ParsedFieldResult {
  fields: Record<number, { fieldNumber: number; name: string; type: FieldSpec['type']; length: number; format: FieldSpec['format']; rawValue: string }>;
  nextOffset: number;
}
...
export function parseFields(
  ...
  const fields: Record<number, { fieldNumber: number; name: string; type: FieldSpec['type']; length: number; format: FieldSpec['format']; rawValue: string }> = {};
